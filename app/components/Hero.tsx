...
export default function Hero() {
  ...
  return (
    <section className="w-full bg-gray-50 dark:bg-gray-900 py-15 transition-colors">
      <Container>
        <div className="grid md:grid-cols-2 gap-10 items-center">
          ...
          <div>
            ...
            <button className="mt-8 px-6 py-3 bg-[#0F2B5A] dark:bg-[#1a3d7a] text-white rounded-lg flex gap-2 items-center hover:bg-[#1a3d7a] dark:hover:bg-[#2a4d8a] transition-all duration-300">
              Upload your resume
        ...
        {/* Grid */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full mx-auto py-5 mt-10">
          {card.map((cards) => (
            <div
              key={cards.name}
              className="group bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer"
            >
              {/* Logo */}
              <div className="flex mb-4">
                <img
                  src={cards.logo}
                  alt={cards.name}
                  className="w-12 h-12 object-contain group-hover:scale-110 transition duration-300 dark:brightness-90"
                />
              </div>

              {/* Name */}
              <p className="font-semibold text-gray-700 dark:text-gray-200 text-xl transition-colors">
                {cards.name}
              </p>
